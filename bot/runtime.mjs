...
const value = (env, primary, fallback) => String(env[primary] || env[fallback] || '').trim();
...
export function botStartConfig(env = process.env) {
  ...
  const domain = value(env, 'OFELIYA_BOT_WEBHOOK_DOMAIN', 'HUB_BOT_WEBHOOK_DOMAIN');
  const webhookPort = port(value(env, 'OFELIYA_BOT_WEBHOOK_PORT', 'HUB_BOT_WEBHOOK_PORT'));
  const path = value(env, 'OFELIYA_BOT_WEBHOOK_PATH', 'HUB_BOT_WEBHOOK_PATH');
  const secret = value(env, 'OFELIYA_BOT_WEBHOOK_SECRET', 'HUB_BOT_WEBHOOK_SECRET');
  const errors = [];
