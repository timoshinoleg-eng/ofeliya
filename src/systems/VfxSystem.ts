...
export class VfxSystem {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    this.killEmitter = scene.add.particles(0, 0, 'spark', {
      speed: { min: 60, max: 190 }, lifespan: { min: 200, max: 420 },
      scale: { start: 1, end: 0 }, blendMode: 'ADD', emitting: false,
    }).setDepth(20);
    this.hitEmitter = scene.add.particles(0, 0, 'spark', {
      speed: { min: 35, max: 95 }, lifespan: { min: 110, max: 190 },
      scale: { start: 0.55, end: 0 }, blendMode: 'ADD', emitting: false,
    }).setDepth(21);
    this.pickupEmitter = scene.add.particles(0, 0, 'spark', {
      speed: { min: 40, max: 110 }, lifespan: 260,
      scale: { start: 0.8, end: 0 }, blendMode: 'ADD', emitting: false,
    }).setDepth(20);
    this.rewardEmitter = scene.add.particles(0, 0, 'spark', {
      speed: { min: 100, max: 290 }, lifespan: { min: 260, max: 540 },
      scale: { start: 1.05, end: 0 }, blendMode: 'ADD', emitting: false,
    }).setDepth(23);
  }
  ...
  kill(x: number, y: number, color: number, importance: KillImportance = 'normal'): void {
    ...
    if (importance !== 'normal') {
      this.ring(x, y, importance === 'boss' ? COLORS.red : COLORS.gold,
        importance === 'boss' ? 84 : 48, 360);
    }
  }
  ...
  singularity(x: number, y: number, radius: number): void {
    ...
    const collapse = this.scene.add.circle(x, y, radius * 0.62)
      .setStrokeStyle(3, COLORS.purple, 0.9).setDepth(19).setBlendMode(Phaser.BlendModes.ADD);
    this.scene.tweens.add({ targets: collapse, scale: 0.12, alpha: 0.1, duration: 170,
      ease: 'Quad.In', onComplete: () => collapse.destroy() });
    const core = this.scene.add.circle(x, y, 7, COLORS.white, 0.8)
      .setDepth(20).setBlendMode(Phaser.BlendModes.ADD);
    this.scene.tweens.add({ targets: core, scale: 2.2, alpha: 0, duration: 320,
      ease: 'Quad.Out', onComplete: () => core.destroy() });
  ...
  destroy(): void {
    this.killEmitter.destroy(); this.hitEmitter.destroy();
    this.pickupEmitter.destroy(); this.rewardEmitter.destroy();
  }
  ...
  private ring(x: number, y: number, color: number, radius: number, duration: number, alpha = 0.22, delay = 0): void {
    const startRadius = 12;
    const ring = this.scene.add.circle(x, y, startRadius, color, alpha)
      .setStrokeStyle(2, color, Math.min(1, alpha * 2.6)).setDepth(19)
      .setBlendMode(Phaser.BlendModes.ADD);
    if (delay > 0) ring.setAlpha(0);
    this.scene.tweens.add({ targets: ring, scale: radius / startRadius,
      alpha: { from: alpha, to: 0 }, duration, delay, ease: 'Quad.Out',
      onStart: () => ring.setAlpha(alpha), onComplete: () => ring.destroy() });
  }
