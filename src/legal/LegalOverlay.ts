...
export function showLegalOverlay(): void {
  ...
  root.innerHTML = `
    ...
    <div class="legal-sheet">
      ...
      <p>Игра использует сервер для рейтингов, дуэлей, Daily-забегов, реферальных функций и технической продуктовой аналитики. Игровые настройки, локальные достижения, настройки звука и история открытий при этом продолжают храниться на устройстве; постоянного серверного профиля персонажа в текущей версии нет.</p>
      ...
      <p>Продуктовая аналитика принимается только из подтверждённых MAX/Telegram-сессий; обычные browser-запуски в неё не записываются. Challenge payload содержит показатели забега и не используется как доверенная авторизация.</p>
