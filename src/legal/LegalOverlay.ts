...
export function showLegalOverlay(): void {
  ...
  root.innerHTML = `
    ...
    <div class="legal-sheet">
      ...
      <p>Продуктовая аналитика принимается только из подтверждённых MAX/Telegram-сессий; обычные browser-запуски в неё не записываются. Показатели забега проходят серверные проверки контракта и базовые анти-чит ограничения перед участием в рейтинговых функциях; текущая версия не выполняет полный серверный replay каждой игровой сессии.</p>
